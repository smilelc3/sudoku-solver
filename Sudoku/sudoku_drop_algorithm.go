...
func DropSolver(sudoku *Sudoku) {
	...
	findBlankCell:
		...
		if hasOnlyCell { // 找到唯一单元格
			...
			// 步骤栈记录所选的单元格和数字
			step := stepRecord{cell, minPossibleNums, sudoku.GetDataByArray()}
...
isExistNoSolutionCell:
		...
		sudoku.LoadDataFromArray(step.sudokuData)
...
// 获取给定单元格所关联的的其他单元格，所占用数字的信息(不重复)
